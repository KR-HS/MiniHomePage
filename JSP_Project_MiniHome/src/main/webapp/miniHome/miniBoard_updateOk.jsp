...
<%@page import="kr.human.board.vo.UpfileVO"%>
...
<body>
	...
	String password="", subject="", content="",category="",userid="";
				...
				if(item.isFormField()){ // 일반폼태그냐?
					...
					case "delfile":
						if(item.getString("UTF-8")!=null && item.getString("UTF-8").trim().length()>0){
							...
							out.println("<script>alert(" + delfile + ");</script>");
						}
						break;
					}
				}else{ // 파일이냐
	...
	<%
		...
		switch(category){
			case "Free":
				switch(m){
					case 1: // 저장
						BoardServiceImpl.getInstance().insert(BoardVO);	
						query = "<script>sendPost(\"miniBoard.jsp"+"\",{\"p\":1,\"b\":"+b+",\"s\":"+s+"});</script>";
						break;	
					case 2: // 수정
						BoardServiceImpl.getInstance().update(BoardVO, application.getRealPath("/upload/"), delfile);
						query = "<script>sendPost(\"miniBoard_view.jsp"+"\",{\"p\":1,\"b\":"+b+",\"s\":"+s+",\"idx\":"+idx+"});</script>";
						break;
					case 3: // 삭제
						BoardServiceImpl.getInstance().delete(BoardVO, application.getRealPath("/upload/"));
						query = "<script>sendPost(\"miniBoard.jsp"+"\",{\"p\":"+p+",\"b\":"+b+",\"s\":"+s+"});</script>";
						break;
				}
				break;
			...
			case "Gallery":
				switch(m){
					case 1: // 저장
						BoardServiceImpl.getInstance().insert(BoardVO);	
						query = "<script>sendPost(\"miniGallery.jsp"+"\",{\"p\":1,\"b\":"+b+",\"s\":"+s+"});</script>";
						break;	
					case 2: // 수정
						BoardServiceImpl.getInstance().update(BoardVO, application.getRealPath("/upload/"), delfile);
						query = "<script>sendPost(\"miniGallery_view.jsp"+"\",{\"p\":1,\"b\":"+b+",\"s\":"+s+",\"idx\":"+idx+"});</script>";
						break;
					case 3: // 삭제
						BoardServiceImpl.getInstance().delete(BoardVO, application.getRealPath("/upload/"));
						query = "<script>sendPost(\"miniGallery.jsp"+"\",{\"p\":"+p+",\"b\":"+b+",\"s\":"+s+"});</script>";
						break;
				}
				break;
			default:
				...
				case 1: // 저장
					BoardServiceImpl.getInstance().insert(BoardVO);	
					query="<script>window.close();</script>";
					break;	
				case 2: // 수정
					BoardServiceImpl.getInstance().update(BoardVO, application.getRealPath("/upload/"), delfile);
					query="<script>window.close();</script>";
