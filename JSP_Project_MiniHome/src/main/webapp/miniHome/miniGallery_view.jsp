...
<%	
	...
	if (BoardVO == null) { //해당글이 없다면
		response.sendRedirect("board_index.jsp");
		return;
	}
	...
	<div class="background">
      <main>
        <jsp:include page="include/profile.jsp">
			...
			<jsp:param name="profile" value="${sessionScope.profile }"/>
		</jsp:include>
                      ...
                      <table>
								<tr>
									<td colspan="4" class="title">
										<h1 style="font-weight:900;">갤러리 내용보기</h1>
									</td>
								</tr>
								<tr >
								...
								<tr>
									...
									<td colspan="3">
										...
										<textarea name="content" id="content" cols="0" rows="3">${content }</textarea>
									</td>
								</tr>
								<tr>
									<td class="item">첨부파일</td>
									<td colspan="3" class="item-content">
										<%-- 첨부 파일 개수를 출력해보자 --%> <c:if
											test="${not empty vo.fileList }">
											<c:forEach var="f" items="${vo.fileList }">
												<div><img src="${pageContext.request.contextPath }/upload/${f.getSfile()}" class="download-image"/>
												<a href="#"
													onclick='sendPost("miniBoard_download.jsp",{"of":"${f.ofile }","sf":"${f.sfile}"});'>
							...
							<br>
							<table class="comment-table" style="width:100%;height:auto;">
								<tr>
									<td colspan="4">
								...
								<tr>
									...
									<td colspan="4" style="border: none;text-align:left;padding:0 0 0 45px">
										<form action="boardCommentUpdate.jsp" method="post" onsubmit="return formCheck();">
											...
											<input type="password" name="password" id="c_password" required="required" placeholder="비밀번호입력"/>
											<br>
										...
										<c:if test="${cv.totalCount>0 }">
											<c:forEach var="cvo" items="${cv.list }">
												<div style="padding:10px; border: 1px solid lightgray;margin-bottom: 5px;">
													<div style="padding:10px; border: none;background-color: #addae2;text-align:left;">
