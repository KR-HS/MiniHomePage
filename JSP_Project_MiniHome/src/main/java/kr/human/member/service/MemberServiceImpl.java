...
public class MemberServiceImpl implements MemberService{
	...
	@Override
	public List<MemberVO> selectList(String name,HttpSession httpSession) {
		log.info("MemberServiceImpl의 selectList호출 : {}", name);
		...
		try {
			...
			//------------------------------------------------------------------
			// 해당 아이디의 회원 정보를 읽어온다
			list = memberDAO.selectByName(sqlSession, name);
			httpSession.setAttribute("memberList", list);
			//------------------------------------------------------------------
			sqlSession.commit();
		}catch (SQLException e) {
			sqlSession.rollback();
		...
		log.info("MemberServiceImpl의 selectList리턴 : {}", list);
		return list;
	}
	...
	@Override
	public void updateIntroduce(MemberVO memberVO, String introduce, HttpSession httpSession) {
		log.info("MemberServiceImpl의 updateIntroduce호출 : {}, {}", memberVO, introduce);
		...
		try {
			...
			//------------------------------------------------------------------
					...
					memberDAO.updateIntroduce(sqlSession, map);
					memberVO=memberDAO.selectByUserid(sqlSession, memberVO.getUserid());
					// 세션의 값을 변경된 값으로 바꿔준다.
					httpSession.setAttribute("memberVO", memberVO);
			//------------------------------------------------------------------
			sqlSession.commit();
	...
	@Override
	public MemberVO searchUser(String userid) {
		log.info("MemberServiceImpl의 searchUser호출 : {}", userid);
		MemberVO memberVO = null;
		...
		log.info("MemberServiceImpl의 searchUser리턴 : {}", memberVO);
		return memberVO;
	}
	@Override
